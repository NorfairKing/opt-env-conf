...
import GHC.Stack (CallStack, HasCallStack, getCallStack, withFrozenCallStack)
...
pureGoldenNixOptions :: (HasCallStack) => FilePath -> Parser a -> GoldenTest Text
pureGoldenNixOptions path parser =
  pureGoldenTextFile path $
    generatedFileComment Stack.callStack <> renderParserNixOptions parser

generatedFileComment :: CallStack -> T.Text
generatedFileComment cs =
  let loc = case getCallStack cs of
        (_, srcLoc) : _ ->
          packageName (Stack.srcLocPackage srcLoc)
            <> ":"
...
-- | Extract just the package name from a GHC package identifier.
--
-- GHC package identifiers look like: @name-version-hash-unitid@
-- This function returns just @name@.
...
packageName = go
  where
    ...
    go ('-' : rest)
      | isVersion version = []
      where
        version = takeWhile (\c -> isDigit c || c == '.') rest
    go (c : rest) = c : go rest
    isVersion v = not (null v) && elem '.' v && all (\c -> isDigit c || c == '.') v
